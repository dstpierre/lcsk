...
public class SqlChatProvider : ChatProvider
{
	...
	public override string RequestChat(ChatRequestInfo request)
	{
        ...
        try
        {
			...
			cmd.Parameters.Add("@Department", SqlDbType.VarChar, 100).Value = request.Department;
	...
	public override List<ChatRequestInfo> GetChatRequests(int operatorId, string[] departments)
	{
        SqlConnection sqlC = new SqlConnection(connectionString);
        ...
        try
        {
			...
			bool found = false;
			List<ChatRequestInfo> toRemove = new List<ChatRequestInfo>();
			foreach (ChatRequestInfo entity in retList)
			{
				found = false;

				foreach (string dep in departments)
				{
					found = dep == entity.Department;
					if (found)
						break;
				}
				...
				if (!found)
				{
					...
					if (entity.RequestDate > DateTime.Now.AddMinutes(-3))
					{
						toRemove.Add(entity);
					}
				}
			...
			foreach (ChatRequestInfo entity in toRemove)
